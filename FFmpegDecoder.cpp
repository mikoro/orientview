...
namespace
{
	...
	int open_codec_context(int* streamIndex, AVFormatContext* formatContext, enum AVMediaType mediaType)
	{
		int result = 0;
...
bool FFmpegDecoder::Open(const std::string& fileName)
{
	...
	try
	{
		...
		if ((result = open_codec_context(&videoStreamIndex, formatContext, AVMEDIA_TYPE_VIDEO)) < 0)
		...
		if ((result = open_codec_context(&audioStreamIndex, formatContext, AVMEDIA_TYPE_AUDIO)) < 0)
		{
...
void FFmpegDecoder::GetNextFrame()
{
}
